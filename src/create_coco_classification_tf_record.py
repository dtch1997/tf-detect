...
def create_tf_examples(image,
  ...
  for i, object_annotations in enumerate(annotations_list):
    if ann_is_valid(object_annotations, image_height, image_width, min_area) and ann_is_person(object_annotations, category_index):      
      ...
      #Create a negative example with same dimensions
      fake_x = np.random.randint(low=0, high=image_width - width+1)
      fake_y = np.random.randint(low=0, high=image_height - height+1)
      cropped_image = image.crop((fake_x, fake_y, fake_x + width, fake_y + height))
      with io.BytesIO() as output:
        cropped_image.save(output, format='jpeg')
        encoded_jpg = output.getvalue()
        key = hashlib.sha256(encoded_jpg).hexdigest()
  ...
  return (keys, examples, num_annotations_skipped)
...
def _create_tf_record_from_coco_annotations(annotations_file, image_dir,
  ...
  with contextlib2.ExitStack() as tf_record_close_stack, \
    ...
    for idx, image in enumerate(images):
      ...
      (_, tf_examples, num_annotations_skipped) = create_tf_examples(
           image, annotations_list, image_dir, category_index, min_area)
      total_num_annotations_skipped += num_annotations_skipped
      
      if tf_examples:
        num_examples = len(tf_examples)
        total_examples += num_examples
        for tf_example in tf_examples:
          total_examples += 1
          shard_idx = total_examples % num_shards
          output_tfrecords[shard_idx].write(tf_example.SerializeToString())
    logging.info('Finished writing, skipped %d annotations.',
                 total_num_annotations_skipped)
    logging.info(f'Total of {total_examples} examples created')
